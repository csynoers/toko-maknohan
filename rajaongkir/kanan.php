...
<table class='table table-bordered'>
			...
			for ($i=0; $i < count($data['rajaongkir']['results']); $i++) { 
				echo "<option value='".$data['rajaongkir']['results'][$i]['city_id']."'>".$data['rajaongkir']['results'][$i]['city_id']." ".$data['rajaongkir']['results'][$i]['city_name']."</option>";
			}
			echo "</select>
						</div>
					  </div>";
			...
			echo "</select>
				...
				<option value='pos'>POS INDONESIA</option>
					  ...
					  <div class='control-group'>
						<div class='controls'>
						<input id='cek' type='submit' value='Cek Ongkir' class='shopBtn'>
...
<h3> Form Checkout</h3>	
	...
	<form action=simpan-transaksi.html method=POST class='form-horizontal'>
      ...
      <option value=0 selected>- -Pilih Kota Anda- -</option>";
  ...
  echo "</select>
		 ...
		 <div class='control-group'>
			...
			<div class='controls'>
			  <input type='text' name='alamat' id='inputLname' placeholder='Masukkan Alamat Anda' required>
			</div>
		 </div>
...
mysql_query("INSERT INTO orders(id_kustomer,id_kota,alamat, tgl_order, jam_order,jenis_pembayaran) 
             VALUES('$_SESSION[kustomer_id]','$_POST[kota]','$_POST[alamat]','$tgl_skrg','$jam_skrg','$_POST[jenis_pembayaran]')");
...
<div class='well well-small'>
	...
	<table>
     <tr><td>Nama Lengkap   </td><td> : <b>$r[nama]</b> </td></tr>
	  ...
	  <tr><td>Jenis Pembayaran         </td><td> : $r[jenis_pembayaran] </td></tr></table><hr /><br />
...
echo"				
  ...
  <input name='' type='submit' value='Bayar Dengan Fasapay' />
			 ...
			 echo"
	<p>Data order dan nomor rekening transfer sudah terkirim ke email Anda.Silahkan lakukan pembayaran ke $z[bank] sebanyak Grand Total yang tercantum, 
         nomor rekeningnya <b>$z[nomor]</b> a.n. $z[atas_nama] <br />
               Apabila Anda tidak melakukan pembayaran dalam 1 hari, maka data order Anda akan terhapus (transaksi batal)</p><br />";
			   }
			   echo"
...
<h3>Konfirmasi Pembayaran</h3>	
	...
	<form action=konfirmasi-aksi.html method=POST enctype='multipart/form-data' class='form-horizontal'>
      ...
      $tampil=mysql_query("SELECT * FROM orders WHERE status_order='Baru' AND id_kustomer='$_SESSION[kustomer_id]' ORDER BY id_orders");
      while($r=mysql_fetch_array($tampil)){
