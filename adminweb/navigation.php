<?php

echo"<li>
          <a href='media.php?module=home'>
            <i class='fa fa-th'></i> <span>Home</span>
            
          </a>
        </li>
		
		<li>
          <a href='media.php?module=admin'>
            <i class='fa fa-users'></i> <span>Admin</span>
            
          </a>
        </li>             
        <li class='treeview'>
          <a href='#'>
            <i class='fa fa-dashboard'></i> <span>Master Data</span> <i class='fa fa-angle-left pull-right'></i>
          </a>
          <ul class='treeview-menu'>
			<li><a href='media.php?module=kategori'><i class='fa fa-circle-o'></i> Kategori</a></li>
            <li><a href='media.php?module=produk'><i class='fa fa-circle-o'></i> Produk</a></li>
			<li><a href='media.php?module=ukuran'><i class='fa fa-circle-o'></i> Ukuran</a></li>
			<li><a href='media.php?module=slide'><i class='fa fa-circle-o'></i> Slide</a></li>
            
            <li><a href='media.php?module=halamanstatis'><i class='fa fa-circle-o'></i> Halaman Statis</a></li>
			
          </ul>
        </li> 
		<li class='treeview'>
          <a href='#'>
            <i class='fa fa-laptop'></i> <span>Manajemen Transaksi</span> <i class='fa fa-angle-left pull-right'></i>
          </a>
          <ul class='treeview-menu'>
		    <li><a href='media.php?module=kustomer'><i class='fa fa-circle-o'></i> Kustomer</a></li>
            <li><a href='media.php?module=order'><i class='fa fa-circle-o'></i> Data Orders</a></li>
			<li><a href='media.php?module=konfirmasi'><i class='fa fa-circle-o'></i> Konfirmasi Pembayaran</a></li>
          </ul>
        </li>
		<li>
          <a href='media.php?module=laporan'>
            <i class='fa fa-book'></i> <span>Laporan</span>
            
          </a>
        </li>
        ";

?>